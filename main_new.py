...
collection = db["pumpfun_raw_tokens"]
...
def process_batch(docs):
    """
    Processes a batch of tokens, fetches the image from the image_uri field, generates descriptions,
    and updates the database.
    """
    for doc in docs:
        token_id = doc["_id"]
        image_url = doc.get("image_uri")

        if not image_url:
            logging.warning(f"Token {token_id} has no image_uri. Skipping.")
